    ...
    when(uiState) {
        is FamilyRewardsUiState.Error -> {
            ErrorScreen(
                ...
                title = (uiState as FamilyRewardsUiState.Error).errorMessageTitle.orEmpty(),
                description = (uiState as FamilyRewardsUiState.Error).errorMessageDescription.orEmpty()
            )
        }
